...
import os
import numpy as np
import builtins # Import builtins to patch open

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

class TestEmbeddingByAITeam(unittest.TestCase):

    def test_embedding_flow(self):
        # Read the script content BEFORE any patching of builtins.open
        script_path = os.path.join(os.path.dirname(__file__), '..', 'indexing', 'embedding_by_AITeam.py')
        original_open = builtins.open # Store original open
        try:
            with original_open(script_path, 'r', encoding='utf-8') as f:
                script_content = f.read()
        finally:
            builtins.open = original_open # Restore original open if something goes wrong
        ...
        mock_torch.cuda.return_value = "cpu" # Default device if not available
        mock_torch.cuda.is_available = MagicMock(return_value=False) # Ensure is_available is a callable mock
        ...
        modules_to_patch = {
            ...
            'dotenv': mock_dotenv,
            'torch': mock_torch, # Use our custom mock_torch
            'numpy': np,
            'os': mock_os, # Use our custom mock_os
        }

        with patch.dict(sys.modules, modules_to_patch):
            # Patch builtins.open specifically for the data file reading within the script
            mock_data_file_open = mock_open(read_data='{"article_id": "1", "clause_id": "1", "content": "test content"}\n')
            with patch('builtins.open', mock_data_file_open):
                with patch.dict(os.environ, {"QDRANT_URL": "mock_url", "QDRANT_API_KEY": "mock_key", "CUDA_VISIBLE_DEVICES": ""}):
                    ...
                    mock_model_cls.assert_called_with("models/AITeamVN")
                    ...
                    self.assertEqual(call_kwargs.get('batch_size'), 16)
                    
                    mock_client_cls.assert_called_with(url="mock_url", api_key="mock_key")
                    
                    mock_client_instance.delete_collection.assert_called_with("legal_clauses_AITeamVN")
                    mock_client_instance.create_collection.assert_called()
                    self.assertEqual(mock_client_instance.create_collection.call_args.kwargs['collection_name'], "legal_clauses_AITeamVN")
