...
class TestEmbeddingByVnDcm(unittest.TestCase):

    def test_embedding_flow(self):
        # Read the script content BEFORE any patching of builtins.open
        script_path = os.path.join(os.path.dirname(__file__), '..', 'indexing', 'embedding_by_vn_dcm_embedding.py')
        ...
        mock_model_instance.get_sentence_embedding_dimension.return_value = 768 # VN_DCM model dim
        ...
        mock_models.Distance.COSINE = "COSINE" # VN_DCM uses COSINE distance
        ...
        with patch.dict(sys.modules, modules_to_patch):
            ...
            mock_data_file_open = mock_open(read_data='{"article_id": "1", "clause_id": "1", "content": "vn content"}\n')
            with patch('builtins.open', mock_data_file_open):
                with patch.dict(os.environ, {"QDRANT_URL": "mock_url", "QDRANT_API_KEY": "mock_key", "CUDA_VISIBLE_DEVICES": ""}):
                    ...
                    # VN_DCM specific: model name, trust_remote_code, device
                    mock_model_cls.assert_called_with(
                        "models/vietnamese-document-embedding",
                        trust_remote_code=True,
                        device="cuda:0" # Hardcoded in the script
                    )
                    ...
                    # VN_DCM script does not call delete_collection
