...
class TestEmbeddingByJina(unittest.TestCase):

    def test_embedding_flow(self):
        # Read the script content BEFORE any patching of builtins.open
        script_path = os.path.join(os.path.dirname(__file__), '..', 'indexing', 'embedding_by_Jina.py')
        ...
        mock_model_instance.get_sentence_embedding_dimension.return_value = 1024 # Jina model dim
        ...
        mock_models.Distance.DOT = "DOT" # Jina uses DOT distance
        ...
        with patch.dict(sys.modules, modules_to_patch):
            ...
            mock_data_file_open = mock_open(read_data='{"article_id": "1", "clause_id": "1", "content": "jina content"}\n')
            with patch('builtins.open', mock_data_file_open):
                with patch.dict(os.environ, {"QDRANT_URL": "mock_url", "QDRANT_API_KEY": "mock_key", "CUDA_VISIBLE_DEVICES": ""}):
                    ...
                    # Jina specific: model name and trust_remote_code
                    ...
                    self.assertEqual(call_kwargs.get('task'), "retrieval.passage") # Jina specific encode param
                    self.assertEqual(call_kwargs.get('batch_size'), 16)
                    
                    mock_client_cls.assert_called_with(url="mock_url", api_key="mock_key")
                    
                    # Jina script does not call delete_collection
