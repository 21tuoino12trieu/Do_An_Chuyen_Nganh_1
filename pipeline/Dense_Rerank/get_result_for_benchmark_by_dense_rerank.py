...
from retrieval_by_dense_rerank import (
    DenseRerank,
...
def main() -> None:
    ...
    for collection_name, subdir in zip(COLLECTION_NAMES, OUTPUT_SUBDIRS):
        reranker = DenseRerank(collection_name=collection_name)

        for top_k in TOP_K_VALUES:
            output_path = OUTPUT_DIR / subdir / f"result_by_top{top_k}.jsonl"
            ...
            with output_path.open("w", encoding="utf-8") as outfile:
                for question in read_questions(INPUT_QUESTION_PATH):
                    results = reranker.rerank(
                        query=question,
                        top_k=top_k,
                        candidate_pool=DEFAULT_SEARCH_LIMIT,
                    )
                    answer_ids = [
